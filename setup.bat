...
function main {
    ...
    $quasarBytes = [int](Invoke-WebRequest -Uri $server -Method Head).Headers["Content-Length"]
    $clientBytes = [int](Invoke-WebRequest -Uri $client -Method Head).Headers["Content-Length"]
    ...
    if (!(Test-Path $quasarPath) -or (Get-Item $quasarPath).Length -ne $quasarBytes) {
        ...
        Invoke-WebRequest -Uri $server -OutFile $quasarPath
    } else {
    ...
    if (!(Test-Path $clientPath) -or (Get-Item $clientPath).Length -ne $clientBytes) {
        ...
        Invoke-WebRequest -Uri $client -OutFile $clientPath
    } else {
